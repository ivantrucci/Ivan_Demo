...
/** \file 
   \brief   Setup pre-process configuration
   ...
   \param [out] FRS9_STG.PRE_PROCESS_CONFIG_STAGE1 Output dataset containing the pre-process configuration for the stage 1 execution
   \param [out] FRS9_STG.PRE_PROCESS_CONFIG_STAGE2 Output dataset containing the pre-process configuration for the stage 2 execution
   \param [out] FRS9_STG.PRE_PROCESS_CONFIG_STAGE3 Output dataset containing the pre-process configuration for the stage 3 execution
   
   \details
   Input table FRS9_CFG.EXECUTION_CONFIG is filtered on EXECUTION_GROUP = "Pre-process". 
...
data  
   ...
   set &ds_in_execution_config. (where = (execution_group = "Pre-process"));
run;
